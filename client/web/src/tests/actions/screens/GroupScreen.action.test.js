...
describe("GroupDetail action", () => {
  ...
  it("test get group actions", async () => {
    ...
    httpMock.onGet(`${process.env.REACT_APP_SERVER_ENDPOINT}api/v1/groups/${groupId}`).reply(200, RETURNED_VALUE);
    ...
    // handle bad path
    store.clearActions();
    httpMock.onGet(`${process.env.REACT_APP_SERVER_ENDPOINT}api/v1/groups/${groupId}`).reply(500);
    await getGroup(groupId)(store.dispatch);

    expect(store.getActions()[0].type).toEqual(GROUP);
    expect(store.getActions()[0].payload).toEqual({ group: [], showErrorModal: true });
  });
  ...
  it("test getSelfMember actions", async () => {
    ...
    httpMock.onGet(`${process.env.REACT_APP_SERVER_ENDPOINT}api/v1/users/email/${userEmail}`).reply(200, {userId});
    httpMock.onGet(`${process.env.REACT_APP_SERVER_ENDPOINT}api/v1/groups/${groupId}/members/${userId}`).reply(200, {groupMembers});

    await getSelfMember(groupId)(store.dispatch);
    expect(store.getActions()[0].type).toEqual(GROUP_MEMBER);
    expect(store.getActions()[0].payload).toEqual({selfMember:groupMembers[0]});

    // test bad path
    store.clearActions();
    httpMock.onGet(`${process.env.REACT_APP_SERVER_ENDPOINT}api/v1/users/email/${userEmail}`).reply(500);
    
    await getSelfMember(groupId)(store.dispatch);
    expect(store.getActions()[0].type).toEqual(GROUP_MEMBER);
    expect(store.getActions()[0].payload).toEqual({ selfMember: null, showErrorModal: true });
  });
  ...
  it("test getOptimalTime actions", async () => {
    ...
    httpMock.onGet(`${process.env.REACT_APP_SERVER_ENDPOINT}api/v1/groups/${groupId}/optimaltime/`).reply(200, {optimalTime});

    await getOptimalTime(groupId)(store.dispatch);
    expect(store.getActions()[0].type).toEqual(OPTIMAL_TIME);
    expect(store.getActions()[0].payload).toEqual({optimalTimes: optimalTime});
  ...
  it("test setOptimalTime actions", async () => {
    ...
    expect(store.getActions()[0].payload).toEqual({meetingModalVisible: false});
    ...
    httpMock.onPost(`${process.env.REACT_APP_SERVER_ENDPOINT}api/v1/groups/meetings/${meetingId}/optimaltime/`, 
    {startTime: "2020-03-01 03:00:00", endTime: "2020-03-01 05:00:00"}).reply(200, {success: true, error: false});
    await setOptimalTime(meetings, selectedMeeting, optimalTime)(store.dispatch);
    expect(store.getActions()[0].type).toEqual(SET_OPTIMAL_TIME);
  ...
  it("test getMeetings actions", async () => {
    ...
    let meetingIds = [{MeetingId: 1, meetingAvailableString}, {MeetingId: 2, meetingAvailableString}, {MeetingId: 3, meetingAvailableString}];
    const stringMeetingIds = "1,2,3";
    const startTime = "2020-03-01 11:30:00";
    const endTime = "2020-03-01 11:30:00";

    httpMock.onGet(`${process.env.REACT_APP_SERVER_ENDPOINT}api/v1/groups/${groupId}/meetings/`).reply(200, {meetingIds});
    httpMock.onGet(`${process.env.REACT_APP_SERVER_ENDPOINT}api/v1/groups/meetings/${stringMeetingIds}/optimaltime/`).reply(200, 
      [{"CAST(StartTime as char)": startTime, "CAST(EndTime as char)": endTime}, {"CAST(StartTime as char)": startTime, "CAST(EndTime as char)": endTime},{"CAST(StartTime as char)": startTime, "CAST(EndTime as char)": endTime}]);
  ...
  it("test showModal actions", async () => {
    ...
    expect(store.getActions()[0].payload).toEqual({meetingModalVisible: true});
    ...
    await showModal(type)(store.dispatch);
    expect(store.getActions()[0].type).toEqual(SHOW_GROUP_DETAIL_MODAL);
    expect(store.getActions()[0].payload).toEqual({meetingModalVisible: false, inputModalVisible: false});
