...
import { getGroupOptimalTime, GET_OPTIMAL_TIMES, selectMeeting, SELECT_MEETING, selectOptimalTime, SELECT_OPTIMAL_TIME, getAllOptimalTimes, toggleMeetingModal, TOGGLE_MEETING_MODAL, setOptimalTime, SET_OPTIMAL_TIME } from '../../src/actions/GetOptimalMeetingTime.action';
...
describe('test get optimal meeting time action', () => {
  ...
  it('test getGroupOptimalTime action', async() => {
      ...
      httpMock.onGet(`${Config.REACT_APP_SERVER_ENDPOINT}api/v1/groups/${groupId}/meetings`).reply(200, {meetingIds: [{MeetingId: 1}, {MeetingId: 2}]});
      httpMock.onGet(`${Config.REACT_APP_SERVER_ENDPOINT}api/v1/groups/meetings/${stringMeetingIds}/optimaltime/`).reply(200, [
      ...
      expect(store.getActions()[0].payload).toEqual({meetings: 
        [
            {
              MeetingId: 1,
              meetingTimeString: 'Date: 2020-04-07 (Tuesday) \n' +
                'Time: 12:00 - 01:00\n' +
  ...
  it('test getAllOptimalTimes action', async() => {
      ...
      httpMock.onGet(`${Config.REACT_APP_SERVER_ENDPOINT}api/v1/groups/${groupId}/optimaltime/`).reply(200, {optimalTime: 1});
  ...
  it('test toggleMeetingModal action', async() => {
      ...
      expect(response.payload.isMeetingModalVisible).toEqual(!isMeetingModalVisible);
  });
  it('test setOptimalTime action', async() => {
      ...
      // handle empty optimalTime
        ...
        expect(store.getActions()[0].payload).toEqual({isMeetingModalVisible: false});
        ...
        store.clearActions();
        httpMock.onPost(`${Config.REACT_APP_SERVER_ENDPOINT}api/v1/groups/meetings/${selectedMeeting.MeetingId}/optimaltime/`).reply(200, {changedTime: false});
        await setOptimalTime(meetings, selectedMeeting, optimalTime)(store.dispatch);
        expect(store.getActions()[0].type).toEqual(SET_OPTIMAL_TIME);
        ...
        store.clearActions();
        httpMock.onPost(`${Config.REACT_APP_SERVER_ENDPOINT}api/v1/groups/meetings/${selectedMeeting.MeetingId}/optimaltime/`).reply(200, {changedTime: true});
        await setOptimalTime(meetings, selectedMeeting, optimalTime)(store.dispatch);
        expect(store.getActions()[0].type).toEqual(SET_OPTIMAL_TIME);
        expect(store.getActions()[0].payload.selectedSameOptimalTime).toEqual(false);
