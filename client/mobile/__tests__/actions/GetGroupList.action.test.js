...
import { getGroupList, GET_GROUP_LIST_SUCCESS } from '../../src/actions/GetGroupList.action';
...
describe('test get group list action', () => {
 ...
 it('test getGroupList action', async() => {
    ...
    httpMock.onGet(`${Config.REACT_APP_SERVER_ENDPOINT}api/v1/groups/`).reply(500);
    await getGroupList()(store.dispatch);
    expect(store.getActions()[0]).toEqual(undefined);

    // handle happy path
    store.clearActions();
    httpMock.onGet(`${Config.REACT_APP_SERVER_ENDPOINT}api/v1/groups/`).reply(200, {groups: [1, 2]});
    await getGroupList()(store.dispatch);
