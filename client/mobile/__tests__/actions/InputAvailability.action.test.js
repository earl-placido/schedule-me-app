...
import { SET_AVAILABILITIES, setAvailabilities, MARK_DATES, SELECT_DATE, selectDate, showModal, SHOW_MODAL, cancelAvailability, handleChangeRangeHour, HANDLE_CHANGE_RANGE_HOUR, addAvailability, ADD_AVAILABILITY, addRangeHour, ADD_RANGE_HOUR, deleteAvailability, DELETE_AVAILABILITY, markDates } from '../../src/actions/InputAvailability.action';
...
describe("test input availability action", () => {
    ...
    it("test setAvailabilities action", async() => {
        ...
        httpMock.onGet(`${Config.REACT_APP_SERVER_ENDPOINT}api/v1/groups/${groupId}/members/${userId}`).reply(200, {groupMembers: [{GroupMemberId: groupMemberId}]});
        httpMock.onGet(`${Config.REACT_APP_SERVER_ENDPOINT}api/v1/groups/members/${groupMemberId}/availability`).reply(200, {error: true});
        ...
        httpMock.onGet(`${Config.REACT_APP_SERVER_ENDPOINT}api/v1/users/email/${userEmail}`).reply(200, {userId});
        httpMock.onGet(`${Config.REACT_APP_SERVER_ENDPOINT}api/v1/groups/${groupId}/members/${userId}`).reply(200, {groupMembers: [{GroupMemberId: groupMemberId}]});
        httpMock.onGet(`${Config.REACT_APP_SERVER_ENDPOINT}api/v1/groups/members/${groupMemberId}/availability`).reply(200, [{'CAST(StartTime as char)': date.format("YYYY-MM-DD HH:mm:ss")},
        {'CAST(StartTime as char)': date.format("YYYY-MM-DD HH:mm:ss")}]);
        await setAvailabilities(groupId)(store.dispatch);
        expect(store.getActions()[0].type).toEqual(MARK_DATES);
        expect(store.getActions()[0].payload).toEqual({markedDates: {[dateFormat]: {marked: true}}});
        expect(store.getActions()[1].type).toEqual(SET_AVAILABILITIES);
        expect(store.getActions()[1].payload.availabilities).toEqual({[dateFormat]: [{'CAST(StartTime as char)': date.format("YYYY-MM-DD HH:mm:ss")},
        {'CAST(StartTime as char)': date.format("YYYY-MM-DD HH:mm:ss")}]});
    ...
    it("test cancelAvailability action", () => {
        ...
        expect(response.type).toEqual(SHOW_MODAL);
        expect(response.payload).toEqual({modalVisible: false, rangeHours: [[1,2,3], [1,2]]});
    });
    ...
    it("test handleChangeRangeHour action", () => {
        ...
        let response = handleChangeRangeHour(selectedTime, selectedDate, index, startOrEndTimeIndex, rangeHours);
        expect(response.type).toEqual(HANDLE_CHANGE_RANGE_HOUR);
        expect(response.payload).toEqual([[]]);

        // handle selectedTime but with startOrEndIndex = 0
        selectedTime = '12:00 PM';
        response = handleChangeRangeHour(selectedTime, selectedDate, index, startOrEndTimeIndex, rangeHours);
        expect(response.type).toEqual(HANDLE_CHANGE_RANGE_HOUR);
        expect(response.payload).toEqual([
            {
              AvailabilityId: 1,
    ...
    it("test addAvailability action", async() => {
        ...
        await addAvailability(groupMemberId, selectedDate, rangeHours, availabilities)(store.dispatch);
        ...
        expect(store.getActions()[0].payload).toEqual({
          ...
          store.clearActions();
          rangeHours = [{AvailabilityId: 1, 'CAST(StartTime as char)': "2020-04-07 12:00:00"}, {AvailabilityId: 2, 'CAST(EndTime as char)': "2020-04-07 12:00:00"}];
          httpMock.onPost(`${Config.REACT_APP_SERVER_ENDPOINT}api/v1/groups/members/availability`).reply(200, {ids: []});
          await addAvailability(groupMemberId, selectedDate, rangeHours, availabilities)(store.dispatch);
          expect(store.getActions()[0].type).toEqual(ADD_AVAILABILITY);
          expect(store.getActions()[0].payload).toEqual({
            modalVisible: false,
          ...
          store.clearActions();
          rangeHours = [{AvailabilityId: 1, 'CAST(StartTime as char)': "2020-04-07 12:00:00"}, {AvailabilityId: 2, 'CAST(EndTime as char)': "2020-04-07 12:00:00"}];
          httpMock.onPost(`${Config.REACT_APP_SERVER_ENDPOINT}api/v1/groups/members/availability`).reply(200, {ids: [1, 2]});
          await addAvailability(groupMemberId, selectedDate, rangeHours, availabilities)(store.dispatch);
          expect(store.getActions()[0].type).toEqual(MARK_DATES);
          expect(store.getActions()[0].payload).toEqual({markedDates: {[dateInString]: {marked: true}}});
          expect(store.getActions()[1].type).toEqual(ADD_AVAILABILITY);
          expect(store.getActions()[1].payload).toEqual({availabilities: {[dateInString]: [{ "AvailabilityId": 1,
                   "CAST(StartTime as char)": "2020-04-07 12:00:00",}, {"AvailabilityId": 2,
                            "CAST(EndTime as char)": "2020-04-07 12:00:00"}]}, error: null, modalVisible: false, rangeHours: [{
                   "AvailabilityId": 1,
    ...
    it("test deleteAvailability action", async() => {
        ...
        await deleteAvailability(rangeHours, availabilities, index, selectedDate)(store.dispatch);
        expect(store.getActions()[0].type).toEqual(DELETE_AVAILABILITY);
        expect(store.getActions()[0].payload).toEqual({ rangeHours: [], availabilities: {} });

        // test error deleting from server
        const AvailabilityId = 1;
        rangeHours = [[{AvailabilityId}]];
        httpMock.onDelete(`${Config.REACT_APP_SERVER_ENDPOINT}api/v1/groups/members/availability`, {availabilityIds: [AvailabilityId]}).reply(200, {error: true});
        await deleteAvailability(rangeHours, availabilities, index, selectedDate)(store.dispatch);
        expect(store.getActions()[0].type).toEqual(DELETE_AVAILABILITY);
        expect(store.getActions()[0].payload).toEqual({ rangeHours: [], availabilities: {} });
        ...
        httpMock.onDelete(`${Config.REACT_APP_SERVER_ENDPOINT}api/v1/groups/members/availability`, {availabilityIds: [AvailabilityId]}).reply(200, {success: true});
        await deleteAvailability(rangeHours, availabilities, index, selectedDate)(store.dispatch);
        expect(store.getActions()[0].type).toEqual(DELETE_AVAILABILITY);
        expect(store.getActions()[0].payload).toEqual({ rangeHours: [], availabilities: {} });
        expect(store.getActions()[1].type).toEqual(DELETE_AVAILABILITY);
    ...
    it("test markDates action", () => {
        ...
        expect(response.type).toEqual(MARK_DATES);
        expect(response.payload).toEqual({markedDates: {"2020-04-07": {marked: true}}});
