...
describe('test get group action', () => {
  ...
  it('test getGroup action', async () => {
      ...
      // test bad path
      httpMock.onGet(`${Config.REACT_APP_SERVER_ENDPOINT}api/v1/groups/${groupId}/`).reply(500, 'failed');
      ...
      // handle happy path
      store.clearActions();
      httpMock.onGet(`${Config.REACT_APP_SERVER_ENDPOINT}api/v1/groups/${groupId}/`).reply(200, 'success');
    await getGroup(groupId)(store.dispatch);
