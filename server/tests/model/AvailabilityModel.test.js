...
const MYSQLDB = {
  ...
  describe("test availability model", () => {
    ...
    it("add availabilities", async() => {
        ...
        const availableInfo = await availabilityModel.addAvailability(groupMemberId, [-1], [formatted_date], [formatted_date]);
    ...
    it("delete availabilities", async() => {
        const groupMemberId = 1;
        const availabilityInfos = await availabilityModel.getAvailability(groupMemberId);
        const currentLength = availabilityInfos.length;
        await availabilityModel.deleteAvailability([1]);
        const newAvailabilityInfos = await availabilityModel.getAvailability(groupMemberId);
